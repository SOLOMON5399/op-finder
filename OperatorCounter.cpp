...
std::map<std::string, int> OperatorCounter::getCombinedOperatorCounts() const
{
    std::map<std::string, int> result;

    // Binary operators
    for (const auto &branch : _binaryOperatorCountsByBranch)
    {
        for (const auto &op : branch.second)
        {
    ...
    // Unary operators
    for (const auto &branch : _unaryOperatorCountsByBranch)
    {
        for (const auto &op : branch.second)
        {
            result[op.first] += op.second;

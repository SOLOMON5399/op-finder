...
bool OperationFinderAstVisitor::VisitBinaryOperator(clang::BinaryOperator* op) {
    ...
    if (_current_function_decl && !is_main) {
        ...
        if (_op_counter) {
            _op_counter->recordBinaryOperator(op_code_str, _op_finder->getCurrentBranch(), multiplier, op_type_name);
        }
    }
...
bool OperationFinderAstVisitor::VisitUnaryOperator(clang::UnaryOperator* op) {
    ...
    if (_current_function_decl && !is_main) {
        ...
        if (_op_counter) {
            _op_counter->recordUnaryOperator(op_code_str, _op_finder->getCurrentBranch(), multiplier, op_type_name);
        }
    }
    _collected_gates++;
    return true;
